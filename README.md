...
This webcomponent shows all available accommodations from opendatahub which are provided with a valid GPS point on a simple map.
...
```html
<webcomp-accommodations xxx="test" yyy="2"></webcomp-accommodations>
```
...
```bash
ToDo: git clone https://github.com/noi-techpark/webcomp-accommodations.git
```

Change directory:

```bash
ToDo: cd webcomp-accommodations/
```
